...
impl KeychainBackend for SystemKeychainBackend {
    fn set_secret(&self, service: &str, account: &str, secret: &[u8]) -> Result<()> {
        let entry = keyring::Entry::new(service, account).map_err(|err| {
            KeySyncError::Vault(format!("failed to open system keychain entry: {err}"))
        })?;
        let encoded = STANDARD.encode(secret);
        entry.set_password(&encoded).map_err(|err| {
            KeySyncError::Vault(format!("failed to save secret to system keychain: {err}"))
        })
    }
    ...
    fn get_secret(&self, service: &str, account: &str) -> Result<Vec<u8>> {
        ...
        let encoded = entry.get_password().map_err(|err| {
            KeySyncError::Vault(format!("failed to read secret from system keychain: {err}"))
        })?;
        STANDARD.decode(encoded).map_err(|err| {
            KeySyncError::Vault(format!("failed to decode system keychain secret: {err}"))
        })
    }
    ...
    fn delete_secret(&self, service: &str, account: &str) -> Result<()> {
        ...
        entry.delete_credential().map_err(|err| {
            KeySyncError::Vault(format!(
                "failed to delete secret from system keychain: {err}"
            ))
        })
    }
}
