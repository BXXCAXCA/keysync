...
impl ProviderAdapter for GeminiAdapter {
    ...
    async fn test_key(&self, config: &ProviderConfig, _api_key: &str, model: Option<&str>) -> Result<TestResult> {
        Ok(TestResult { ok: false, provider_id: config.id.clone(), model_count: Some(0), selected_model: model.map(str::to_owned), latency_ms: None, message: "Gemini adapter placeholder is wired".into() })
    }
